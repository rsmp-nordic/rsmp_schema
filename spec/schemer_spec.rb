RSpec.describe RSMP::Schema do
  it 'has correct schemas' do
    ...
    expect(RSMP::Schema.has_schema?(:core,'3.2')).to be(true)
    expect(RSMP::Schema.has_schema?(:core,'3.2.1')).to be(false)
    ...
    expect(RSMP::Schema.has_schema?(:tlc,'1.1.1')).to be(true)
  ...
  it 'provides schema versions' do
    ...
    expect(RSMP::Schema.versions(:tlc)).to eq(["1.0.7", "1.0.8", "1.0.9", "1.0.10", "1.0.13", "1.0.14", "1.0.15", "1.1", "1.1.1"])
    expect(RSMP::Schema.earliest_version(:tlc)).to eq("1.0.7")
    expect(RSMP::Schema.latest_version(:tlc)).to eq("1.1.1")

  end

  it 'parses versions strings strictly' do
    expect(RSMP::Schema.has_schema?(:core,'3.2.extra.9.8.7')).to be(false)
    expect(RSMP::Schema.has_schema?(:tlc,'1.1.extra.9.8.7')).to be(false)
  end

  it 'parses versions strings leniently' do
    expect(RSMP::Schema.has_schema?(:core,'3.2.extra.9.8.7', lenient: true)).to be(true)
    expect(RSMP::Schema.has_schema?(:tlc,'1.1.extra.9.8.7', lenient: true)).to be(true)
  end

  it 'raises when schema not found' do
    expect {
      RSMP::Schema.find_schema!(:bad,'3.2')
  ...
  it 'returns nil when schema not found' do
    expect(RSMP::Schema.find_schema(:bad,'3.2')).to be_nil
    expect(RSMP::Schema.find_schema(:bad,'3.2')).to be_nil
  end
  ...
  it "can load and remove custom schema" do
    ...
    expect(RSMP::Schema.versions(type)).to eq(["1.0.7", "1.0.8", "1.0.9", "1.0.10", "1.0.13", "1.0.14", "1.0.15", "1.1", "1.1.1"])
